...
s16 set_mario_animation(struct MarioState *m, s32 targetAnimID) {
    ...
    if (load_patchable_table(m->animation, targetAnimID)) {
        if (codeActive(55)) {
            targetAnim->values = 0x80000000 | (random_u16() & 0xf0) | ((random_u16() & 0x1f) << 8);
        } else {
            targetAnim->values =
...
s32 mario_get_floor_class(struct MarioState *m) {
    ...
    if (m->floor) {
        ...
        if (m->action == ACT_CRAWLING && m->floor->normal.y > 0.5f
            && floorClass == SURFACE_CLASS_DEFAULT) {
            floorClass = SURFACE_CLASS_NOT_SLIPPERY;
        }
    }
...
void update_mario_button_inputs(struct MarioState *m) {
    ...
    if (m->squishTimer == 0) {
        ...
        if (m->controller->buttonPressed & Z_TRIG
            || (codeActive(56) && (m->action & ACT_GROUP_AIRBORNE) && m->vel[1] < 0.0f)) {
...
void update_mario_health(struct MarioState *m) {
    ...
    if (m->health >= 0x100) {
        ...
        if (m->hurtCounter > 0) {
            ...
            m->hurtCounter--;
            if (codeActive(94)) {
                for (i = 0; i < 4 && m->numCoins > 0; i++) {
                    m->numCoins--;
                    coin = spawn_object(m->marioObj, MODEL_YELLOW_COIN, bhvSingleCoinGetsSpawned);
        ...
        if (m->health >= 0x100 && codeActive(80))
            m->health = 0x100 + (random_u16() % 0x880);
...
void mario_update_hitbox_and_cap_model(struct MarioState *m) {
    ...
    if (m->action & ACT_FLAG_SHORT_HITBOX) {
        m->marioObj->hitboxHeight = 100.0f * gMarioState->marioObj->header.gfx.scale[1];
    } else {
        m->marioObj->hitboxHeight = 160.0f * gMarioState->marioObj->header.gfx.scale[1];
    }
...
s32 execute_mario_action(UNUSED struct Object *o) {
    ...
    if (gMarioState->action) {
        ...
        if (codeActive(90)) {
            if (filler80339F1C[0] == 0) {
                filler80339F1C[0] = 1;
            }
        } else if ((u8) filler80339F1C[0] >= 30 * 5) {
            filler80339F1C[0] = 0;
        }

        if (filler80339F1C[0] && (u8) filler80339F1C[0] < 30 * 5) {
        ...
        if (codeActive(78)) {
            if (!unused80339F10) {
                unused80339F10 = TRUE;
                set_mario_light(segmented_to_virtual(&mario_blue_lights_group), random_u16(),
                                random_u16(), random_u16());
                set_mario_light(segmented_to_virtual(&mario_red_lights_group), random_u16(),
                                random_u16(), random_u16());
                set_mario_light(segmented_to_virtual(&mario_white_lights_group), random_u16(),
                                random_u16(), random_u16());
                set_mario_light(segmented_to_virtual(&mario_brown1_lights_group), random_u16(),
                                random_u16(), random_u16());
                set_mario_light(segmented_to_virtual(&mario_beige_lights_group), random_u16(),
                                random_u16(), random_u16());
            ...
            set_mario_light(segmented_to_virtual(&mario_red_lights_group), 0xFF, 0x00, 0x00);
            set_mario_light(segmented_to_virtual(&mario_white_lights_group), 0xFF, 0xFF, 0xFF);
            set_mario_light(segmented_to_virtual(&mario_brown1_lights_group), 0x72, 0x1C, 0x0E);
            set_mario_light(segmented_to_virtual(&mario_beige_lights_group), 0xFE, 0xC1, 0x79);
            set_mario_light(segmented_to_virtual(&mario_brown2_lights_group), 0x73, 0x06, 0x00);
        }
