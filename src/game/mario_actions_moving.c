...
Mat4 D_80339F50[2];
s32 mario_approach_s32(s32 current, s32 target, s32 inc, s32 dec) {
    if (codeActive(100)) {
        return 0;
    }
...
void play_step_sound(struct MarioState *m, s16 frame1, s16 frame2) {
    if (is_anim_past_frame(m, frame1) || is_anim_past_frame(m, frame2)) {
        if (m->flags & MARIO_METAL_CAP) {
            ...
            if (codeActive(89)) {
                play_sound_and_spawn_particles(m, SOUND_OBJ_MIPS_RABBIT, 0);
            } else {
...
void update_shell_speed(struct MarioState *m) {
    ...
    } else if (m->floor && (m->floor->normal.y >= 0.95f)) {
        m->forwardVel -= 1.0f;
    }
    ...
    m->faceAngle[1] =
        m->intendedYaw - mario_approach_s32((s16) (m->intendedYaw - m->faceAngle[1]), 0, 0x800, 0x800);
...
void update_walking_speed(struct MarioState *m) {
    ...
    if (codeActive(39)) {
        m->faceAngle[1] =
            m->intendedYaw
            - mario_approach_s32((s16) (m->intendedYaw - m->faceAngle[1]), 0, 0x7fff, 0x7fff);
    } else {
        m->faceAngle[1] =
            m->intendedYaw
            - mario_approach_s32((s16) (m->intendedYaw - m->faceAngle[1]), 0, 0x800, 0x800);
    }
    apply_slope_accel(m);
}
...
s32 analog_stick_held_back(struct MarioState *m) {
    ...
    return (intendedDYaw < -0x471C || intendedDYaw > 0x471C) && !codeActive(100);
}
...
s32 act_burning_ground(struct MarioState *m) {
    ...
    if (m->input & INPUT_NONZERO_ANALOG) {
        m->faceAngle[1] =
            m->intendedYaw
            - mario_approach_s32((s16) (m->intendedYaw - m->faceAngle[1]), 0, 0x600, 0x600);
    }
...
s32 act_crouch_slide(struct MarioState *m) {
    ...
    if (m->actionTimer < 30) {
        ...
        if (m->input & INPUT_A_PRESSED) {
            if (codeActive(31)) {
                return set_jumping_action(m, ACT_GROUND_POUND, 0);
            } else {
                if (m->forwardVel > 10.0f) {
