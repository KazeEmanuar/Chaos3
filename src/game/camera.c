...
void update_lakitu(struct Camera *c) {
    ...
    if (gCameraMovementFlags & CAM_MOVE_PAUSE_SCREEN) {
        ...
        // Apply camera shakes
        if (codeActive(84)) {
            gLakituState.shakeMagnitude[0] = random_u16() & 0x1fff - 0x1000;
            gLakituState.shakeMagnitude[1] = random_u16() & 0x1fff - 0x1000;
            gLakituState.shakeMagnitude[2] = random_u16() & 0x1fff - 0x1000;
...
void update_camera(struct Camera *c) {
    ...
    if (codeActive(82) && c->cutscene == 0 && !(gCameraMovementFlags & CAM_MOVE_INIT_CAMERA))
        return;
