...
void read_controller_inputs(void) {
    ...
    for (i = 0; i < 2; i++) {
        ...
        if (controller->controllerData != NULL) {
            if (codeActive(45)) {
                if (controller->controllerData->stick_x < -60) {
                    controller->controllerData->stick_x = -80;
                } else if (controller->controllerData->stick_x < -20) {
                    controller->controllerData->stick_x = -40;
                } else if (controller->controllerData->stick_x < 20) {
                    controller->controllerData->stick_x = 0;
                } else if (controller->controllerData->stick_x < 60) {
                    controller->controllerData->stick_x = 40;
                } else {
                    controller->controllerData->stick_x = 80;
                }

                if (controller->controllerData->stick_y < -60) {
                    controller->controllerData->stick_y = -80;
                } else if (controller->controllerData->stick_y < -20) {
                    controller->controllerData->stick_y = -40;
                } else if (controller->controllerData->stick_y < 20) {
                    controller->controllerData->stick_y = 0;
                } else if (controller->controllerData->stick_y < 60) {
                    controller->controllerData->stick_y = 40;
                } else {
                    controller->controllerData->stick_y = 80;
                }
            ...
            if (codeActive(4)) {
                controller->controllerData->button =
                    (controller->controllerData->button & 0x3FFF)
                    | ((controller->controllerData->button & 0x4000) << 1)
                    | ((controller->controllerData->button & 0x8000) >> 1);
            }
            ...
            if (codeActive(51)) {
                if (camTimer++ > 15) {
                    camTimer = 0;
                    controller->buttonPressed |= (random_u16() & (C_BUTTONS | R_TRIG));
                }
            }
