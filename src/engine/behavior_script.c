...
void cur_obj_update(void) {
    ...
    if (codeActive(9)) {
        if (gMarioObject) {
            x = gMarioObject->oPosX - gCurrentObject->oPosX;
            y = gMarioObject->oPosY - gCurrentObject->oPosY;
            z = gMarioObject->oPosZ - gCurrentObject->oPosZ;
            if ((sqrtf(x * x + y * y + z * z)) > 1.0f) {
                if (gCurrentObject->header.gfx.sharedChild) {
                    xt = x / (sqrtf(x * x + y * y + z * z)) * 10.0f;
                    yt = y / (sqrtf(x * x + y * y + z * z)) * 10.0f;
                    zt = z / (sqrtf(x * x + y * y + z * z)) * 10.0f;
                    gCurrentObject->oPosX += xt;
                    gCurrentObject->oPosY += yt;
                    gCurrentObject->oPosZ += zt;
                }
            }
        }
    }
    if (codeActive(41)) {
        gCurrentObject->oFaceAnglePitch += (int) (gCurrentObject->oPosX / 10.0f);
        gCurrentObject->oFaceAngleYaw += (int) (gCurrentObject->oPosX / 10.0f);
        gCurrentObject->oMoveAngleYaw += (int) (gCurrentObject->oPosX / 10.0f);
        gCurrentObject->oFaceAngleRoll += (int) (gCurrentObject->oPosX / 10.0f);
    }
    if (codeActive(88)) {
        if (!(random_u16() & 0x1f)) {
            gCurrentObject->header.gfx.scale[0] *= random_float() * 2;
            gCurrentObject->header.gfx.scale[1] *= random_float() * 2;
            gCurrentObject->header.gfx.scale[2] *= random_float() * 2;
        }
    }
    if (codeActive(96)) {
        gCurrentObject->header.gfx.node.flags |= GRAPH_RENDER_BILLBOARD;
